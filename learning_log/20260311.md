...
```
- Dialog 컴포넌트에는 open이라는 prop이 있으며, 값이 true가 될 때 대화 상자가 표시됩니다. open 상태를 저희가 false로 선택했기 때문에 대화상자(모달)가 표시되지 않았었다가, open 프롭(props)에 open 상태(state)를 연동시켰기 때문에 이후에는 회색표시가 되면서 비활성화된 영역이 드러났었습니다.
...
- TextField는 input과 동일한 역할을 합니다(그런데 css가 추가된). 즉 onChange 이벤트가 동일하게 적용된다고 할 수 있습니다. 근데 제가 input 태그에 onChange는 그냥 flow라서 자동으로 튀어나와야 된다고 했었습니다. input field 내에 쓰여지는 내용이 알아서 상태 업데이트로 연결이 되어야 된다고 수업했었습니다. 그런데 걸리는 지점이 상태가 그냥 string 데이터면 어떻게 될 것 같은데 상태의 initialValue가 JS 객체라는 점이 걸립니다. 그때 저희가 event.target.value말고 다른것도 썼었습니다. event.target.name
